...
class GaussPredictorMLP(nn.Module):
    def __init__(self, input_image_features_dim, hidden_dim=512, positional_encoding_dim=20, latent_codes={}):
        ...
        # Latent Codes
        self.latent_codes_enabled = latent_codes.get('enabled', False)
        self.latent_code_size = latent_codes['latent_code_size'] if self.latent_codes_enabled else 0
        
        # Input dim
        self.input_dim = self.xyz_dim_in + self.scaling_dim + self.rotation_dim + self.opacity_dim + self.color_dim + input_image_features_dim + self.latent_code_size
    ...
    def forward(self, input):    
        ...
        if self.latent_codes_enabled:
            input_list.append(input['latent_codes'])
        
        mlp_input = torch.cat(input_list, dim=-1).to('cuda')
