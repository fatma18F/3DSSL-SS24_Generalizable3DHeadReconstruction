...
def vertices2landmarks(vertices, faces, lmk_faces_idx, lmk_bary_coords):
    ...
    landmarks = torch.einsum("blfi,blf->bli", [lmk_vertices.double(), lmk_bary_coords.double()])
    return landmarks
...
def lbs(
    ...
    if pose2rot:
        ...
        pose_offsets = torch.matmul(pose_feature.double(), posedirs.double()).view(batch_size, -1, 3)
    else:
    ...
    T = torch.matmul(W.double(), A.double().view(batch_size, num_joints, 16)).view(batch_size, -1, 4, 4)
...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    rel_transforms = transforms.double() - F.pad(
        torch.matmul(transforms.double(), joints_homogen.double()), [3, 0, 0, 0, 0, 0, 0, 0]
    )
