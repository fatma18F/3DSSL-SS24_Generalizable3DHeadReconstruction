...
class GaussPredictorMLPLarge(nn.Module):
    def __init__(self, input_image_features_dim, hidden_dim=512, positional_encoding_dim=20,latent_codes=None,static_color=None,positional_encoding_color=False):
        super(GaussPredictorMLPLarge, self).__init__()
        ...
        self.mlp1.apply(GaussPredictorMLPLarge.he_weight_init)
        self.mlp2.apply(GaussPredictorMLPLarge.he_weight_init)
        self.mlp3.apply(GaussPredictorMLPLarge.he_weight_init)
    ...
    def forward(self, input):    
        ...
        mlp_output = torch.cat([mlp_input, mlp_output], dim=-1).to('cuda')
        mlp_output = self.mlp3(mlp_output)
        mlp_output = torch.cat([mlp_input, mlp_output], dim=-1).to('cuda')
